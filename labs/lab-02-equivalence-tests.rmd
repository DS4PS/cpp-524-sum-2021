...
*Camarena, L., Feeney, M. K., & Lecy, J. (2021). Nonprofit Entrepreneurship: Gender Differences in Strategy and Practice. Nonprofit and Voluntary Sector Quarterly.* [**PDF**](../pubs/camarena-et-al-2021.pdf)
...
When examining group composition it is common to include traits like race and gender that are categorical and not numerical measures. 
...
What do we mean by a test for the independence of two variables? 
...
Heart rate depends on caffeine intake if the two variables are related. 
...
When factors are independent then knowing additional information about X tells us nothing about Y. In the case on the left we could do just as well guessing the heart rate for each individual using the average for the entire group instead of using the regression line. 

When factors are dependent, then information about X should tell us something about the expected value of Y. The regression line on the right is a much better estimate of heart rate for most cases than the average heart rate of the group (the blue line). 
...
So let's apply this same predictive reasoning to understand the meaning of a test for independence for categorical variables.
...
![](figures/contingency-table.png)

We then want to use a chi-square technique to test for independence. 
...
![](figures/chi-square-residuals.png)
...
To examine the relationship between the two variables we would create a cross-tab of the two traits using `table( hair, eyes )` to produce one similar to the table stored in the vcd package:
...
{
  df <- min( n1, n2 ) - 1  # size of treated group and comparison
...
[ ![](figures/download.png) ](https://raw.githubusercontent.com/DS4PS/cpp-524-sum-2021/main/labs/lab-02-template.rmd)
...
# Extended Example: Designing a Decision Criteria
...
for( k in 0:10 )
{
x[k+1] <- choose( n=n, k=k ) * ( p ^ k  ) * ( (1-p) ^ (n-k) )
}

d <- data.frame( heads=(0:10), prob=round(x,3) )
